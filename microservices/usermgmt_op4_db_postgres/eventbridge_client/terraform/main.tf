...
resource "aws_cloudwatch_event_target" "the_target" {
  ...
  input     = "{\"commands\":[\"export server_address=${var.server_private_ip}\",\"export HOME=/home/ubuntu\",\"export GOPATH=$HOME/go\",\"export GOMODCACHE=$HOME/go/pkg/mod\",\"export GOCACHE=$HOME/.cache/go-build\",\"cd /home/ubuntu/\",\"cd golang_aws_terraform_jenkins\",\"cd microservices/usermgmt_op4_db_postgres/usermgmt_client\",\"sudo chmod 700 usermgmt_client\",\"sudo --preserve-env ./usermgmt_client\"]}"
