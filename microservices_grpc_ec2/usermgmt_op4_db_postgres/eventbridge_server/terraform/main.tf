...
resource "aws_cloudwatch_event_target" "the_target" {
  ...
  //input     = "{\"commands\":[\"ls -a\"]}"
  input     = "{\"commands\":[\"export db_conn=${local.db_conn}\",\"export db_password=${var.db_password}\",\"export HOME=/home/ubuntu\",\"export GOPATH=$HOME/go\",\"export GOMODCACHE=$HOME/go/pkg/mod\",\"export GOCACHE=$HOME/.cache/go-build\",\"cd /home/ubuntu/\",\"cd golang_aws_terraform_jenkins\",\"cd microservices_grpc_ec2/usermgmt_op4_db_postgres/usermgmt_server\",\"sudo chmod 700 usermgmt_server\",\"sudo --preserve-env ./usermgmt_server\"]}"
