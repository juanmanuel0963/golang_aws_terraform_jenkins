...
resource "aws_cloudwatch_event_target" "the_target" {
  ...
  input     = "{\"commands\":[\"cd /home/ubuntu/\",\"cd golang_aws_terraform_jenkins/microservices_restful_ec2/blogs/source_code/tls\",\"sudo chmod 700 ./\",\"go run $(go env GOROOT)/src/crypto/tls/generate_cert.go -rsa-bits 2048 -host localhost\",\"cd ..\",\"export PORT=${var.blogs_port}\",\"export db_conn='${local.db_conn}'\",\"sudo chmod 700 main\",\"sudo --preserve-env ./main\"]}"
