...
variable "aws_cognito_car_pool_id"{
  type    = string
}
...
resource "aws_cloudwatch_event_target" "the_target" {
  ...
  input     = "{\"commands\":[\"cd /home/ubuntu/\",\"cd golang_aws_terraform_jenkins/microservices_restful_ec2/cars/source_code\",\"export PORT=${var.cars_port}\",\"export db_conn='${local.db_conn}'\",\"export region='${var.region}'\",\"export aws_cognito_user_pool_id='${var.aws_cognito_car_pool_id}'\",\"sudo chmod 777 main\",\"sudo --preserve-env ./main\"]}"
